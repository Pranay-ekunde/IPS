...
exports.getAll = async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT i.id, i.name, i.purchase_date, i.stock_available, i.active,
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      ...
      [name.trim(), item_type_id, purchase_date, stockVal, active ? 1 : 0]
    );
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      'UPDATE items SET name=?, item_type_id=?, purchase_date=?, stock_available=?, active=? WHERE id=?',
      [name.trim(), item_type_id, purchase_date, stockVal, active ? 1 : 0, id]
    );
